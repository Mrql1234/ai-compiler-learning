...
建议在 `projects/triton-kernel-library/triton-learning/` 下新增如下目录：
...
- 与 `triton-kernel-library` 现有 `kernels/`、`tests/`、`benchmarks/` 主线分层清晰
- 把“作品集训练代码”和当前库内已有算子实现隔离开
- 后续更容易单独展示、迁移或抽成面试作品
...
```bash
cd projects/triton-kernel-library/triton-learning
