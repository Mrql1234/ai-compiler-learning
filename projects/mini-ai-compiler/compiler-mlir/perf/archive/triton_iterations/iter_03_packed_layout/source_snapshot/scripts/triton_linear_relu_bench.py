...
def make_kernel(tl: Any) -> Any:
    ...
    def kernel(
        ...
        for k_start in range(0, tl.cdiv(k, BLOCK_K)):
            ...
            acc += tl.dot(input_tile, weight_tile, input_precision="ieee")
...
def pack_weight_for_kernel(weight_tensor: Any) -> Any:
    # Pack the static linear weight into a KxN contiguous layout that matches
    # the Triton matmul-style access pattern better than the original NxK layout.
    return weight_tensor.t().contiguous()
...
def run_kernel(
    ...
    packed_weight_tensor = pack_weight_for_kernel(weight_tensor)
    ...
    def launch() -> None:
        kernel[grid](
            ...
            packed_weight_tensor.stride(1),
            packed_weight_tensor.stride(0),
