...
def _resolve_base_candidates() -> tuple[Path, ...]:
...
def _existing_prefix_depth(base: Path, candidate: Path) -> int:
    current = base
    depth = 0
...
def resolve_input_path(path: str | Path) -> Path:
    ...
    if candidate.is_absolute():
        return candidate
    for base in _resolve_base_candidates():
...
def resolve_output_path(path: str | Path) -> Path:
    ...
    if candidate.is_absolute():
        return candidate
    best_base = PROJECT_DIR.resolve()
    best_depth = -1
    for base in _resolve_base_candidates():
        depth = _existing_prefix_depth(base, candidate)
        if depth > best_depth:
            best_base = base
            best_depth = depth
    return (best_base / candidate).resolve()
